...
الطريقة بالتفصيل (المراحل، الملفات بينها، الحدود، البوابات، وإزاي تضيف نوع رسمة جديد) في [`FLOW.md`](FLOW.md).
...
2. `sheets.py`: تحديد المساقط (عناوين، فقاعات، أو أقسام بعناوين كبيرة)، وتقسيم المباني المنفصلة.
...
- `test_pipeline.py`: مسقط صناعي (`tests/make_struct_sample.py`) معروف الحقيقة، بيتحوّل من الأول للآخر. الناتج لازم يطابقها في أربع حالات:
  ...
  وفيه كمان:
  - اختبار إن الفحص بيمسك عمود برّه البلاطة وكمرة مكررة؛
  - بلاطة مسطحة بدروبات مرسومة مستطيلات وملاحظة عمود مزروع؛
  - رسمة أقسام (FRAMING / LOADING) فيها مبنيين في كل دور، ودور متوصّل بخطوط لازم يتقسم زي التاني.
